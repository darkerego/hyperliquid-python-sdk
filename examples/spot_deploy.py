...
async def main():
    ...
    register_token_result = await exchange.spot_deploy_register_token(
        "TEST0", 2, 8, 1000000000000, "Test token example"
    )
    ...
    user_genesis_result = await exchange.spot_deploy_user_genesis(
        token,
        [
    ...
    # No-op
    user_genesis_result = await exchange.spot_deploy_user_genesis(token, [], [])
    print(user_genesis_result)
    # Distribute 100000000000000 wei on a weighted basis to all holders of token with index 1
    user_genesis_result = await exchange.spot_deploy_user_genesis(token, [], [(1, "100000000000000")])
    print(user_genesis_result)
    ...
    if ENABLE_FREEZE_PRIVILEGE:
        ...
        enable_freeze_privilege_result = await exchange.spot_deploy_enable_freeze_privilege(token)
        print(enable_freeze_privilege_result)

        # Freeze user for token
        freeze_user_result = await exchange.spot_deploy_freeze_user(token, DUMMY_USER, True)
        print(freeze_user_result)

        # Unfreeze user for token
        unfreeze_user_result = await exchange.spot_deploy_freeze_user(token, DUMMY_USER, False)
    ...
    genesis_result = await exchange.spot_deploy_genesis(token, "300000000000000", False)
    ...
    register_spot_result = await exchange.spot_deploy_register_spot(token, 0)
    ...
    register_hyperliquidity_result = await exchange.spot_deploy_register_hyperliquidity(spot, 2.0, 4.0, 100, None)
    ...
    if SET_DEPLOYER_TRADING_FEE_SHARE:
        ...
        set_deployer_trading_fee_share_result = await exchange.spot_deploy_set_deployer_trading_fee_share(
            token, "100%"
        )
    ...
    if ENABLE_QUOTE_TOKEN:
        ...
        enable_quote_token_result = await exchange.spot_deploy_enable_quote_token(token)
