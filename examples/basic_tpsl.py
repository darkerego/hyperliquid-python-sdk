...
async def main():
    ...
    _, _, exchange = await example_utils.setup(constants.TESTNET_API_URL, skip_ws=True)
    ...
    order_result = await exchange.order(coin, is_buy, sz, px, {"limit": {"tif": "Gtc"}})
    print(order_result)

    # Place a stop order
    stop_order_type = {"trigger": {"triggerPx": trigger_px, "isMarket": True, "tpsl": "sl"}}
    stop_result = await exchange.order("ETH", not is_buy, sz, sl_px, stop_order_type, reduce_only=True)
    print(stop_result)
    ...
    tp_result = await exchange.order("ETH", not is_buy, sz, px, tp_order_type, reduce_only=True)
    print(tp_result)
    ...
    resp = await exchange.bulk_orders(orders, grouping="normalTpsl")
    print(resp)
