...
async def main():
    ...
    order_result = await exchange.order("ETH", True, 0.2, 1100, {"limit": {"tif": "Gtc"}}, cloid=cloid)
    print(order_result)

    # Query the order status by cloid
    order_status = await info.query_order_by_cloid(address, cloid)
    print("Order status by cloid:", order_status)

    # Non-existent cloid example
    invalid_cloid = Cloid.from_int(2)
    order_status = await info.query_order_by_cloid(address, invalid_cloid)
    print("Order status by cloid:", order_status)
    ...
    if order_result["status"] == "ok":
        ...
        if "resting" in status:
            cancel_result = await exchange.cancel_by_cloid("ETH", cloid)
            print(cancel_result)
