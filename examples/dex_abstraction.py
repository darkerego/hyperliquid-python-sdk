...
async def main():
    ...
    agent_enable_dex_abstraction_result = await exchange.agent_enable_dex_abstraction()
    ...
    if user == exchange.wallet.address:
        ...
        user_dex_abstraction_result = await exchange.user_dex_abstraction(user, False)
        print(user_dex_abstraction_result)
        print("current user dex abstraction state:", await info.query_user_dex_abstraction_state(user))

        # enable and disable dex abstraction for sub-account of user
        sub_accounts = await info.query_sub_accounts(user)
        ...
        for sub_account in sub_accounts:
            if sub_account["name"] == SUB_ACCOUNT_NAME:
                ...
                exchange_with_sub_account = await Exchange.create(
                    exchange.wallet, exchange.base_url, vault_address=sub_account_user
                )
                agent_enable_dex_abstraction_result = await exchange_with_sub_account.agent_enable_dex_abstraction()
                print("sub-account agent_enable_dex_abstraction result:", agent_enable_dex_abstraction_result)

                for enabled in [True, False]:
                    user_dex_abstraction_result = await exchange.user_dex_abstraction(sub_account_user, enabled)
                    print(user_dex_abstraction_result)
                    print(
                        "current sub-account user dex abstraction state:",
                        await info.query_user_dex_abstraction_state(sub_account_user),
                    )
