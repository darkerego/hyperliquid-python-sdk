...
async def main():
    ...
    if order_result["status"] == "ok":
        ...
        if "resting" in status:
            ...
            order_status = await info.query_order_by_oid(address, oid)
            print("Order status by oid:", order_status)

            modify_result = await exchange.modify_order(
                oid, "ETH", True, 0.1, 1105, {"limit": {"tif": "Gtc"}}, cloid=cloid
            )
            print("modify result with oid:", modify_result)

            modify_result = await exchange.modify_order(cloid, "ETH", True, 0.1, 1105, {"limit": {"tif": "Gtc"}})
