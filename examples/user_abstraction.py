...
async def main():
    ...
    print("current user abstraction state:", await info.query_user_abstraction_state(user))
    agent_set_abstraction_result = await exchange.agent_set_abstraction("u")
    print(agent_set_abstraction_result)

    if user == exchange.wallet.address:
        # set abstraction for user back to disabled
        user_set_abstraction_result = await exchange.user_set_abstraction(user, "unifiedAccount")
        ...
        exchange_with_sub_account = await Exchange.create(exchange.wallet, exchange.base_url, vault_address=SUB_ACCOUNT_USER)
        agent_set_abstraction_result = await exchange_with_sub_account.agent_set_abstraction("u")
        print("sub-account agent_set_abstraction result:", agent_set_abstraction_result)

        for abstraction in ["disabled", "portfolioMargin"]:
            user_set_abstraction_result = await exchange.user_set_abstraction(SUB_ACCOUNT_USER, abstraction)
            print(user_set_abstraction_result)
            print(
                "current sub-account user abstraction state:",
                await info.query_user_abstraction_state(SUB_ACCOUNT_USER),
            )
