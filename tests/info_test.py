...
async def test_get_user_state():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_state("0x5e9ee1089755c3435139848e47e6635505d5a13a")
...
async def test_get_open_orders():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.open_orders("0x5e9ee1089755c3435139848e47e6635505d5a13a")
    assert len(response) == 196


@pytest.mark.vcr()
async def test_get_frontend_open_orders():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.frontend_open_orders("0xCB331197E84f135AB9Ed6FB51Cd9757c0bd29d0D")
...
async def test_get_user_fills():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_fills("0xb7b6f3cea3f66bf525f5d8f965f6dbf6d9b017b2")
    assert isinstance(response, list)
    assert response[0]["crossed"] is True


@pytest.mark.vcr()
async def test_get_user_fills_by_time():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_fills_by_time(
...
async def test_get_funding_history(endTime):
    async with Info(skip_ws=True, spot_meta=TEST_SPOT_META) as info:
        if endTime is None:
            response = await info.funding_history(name="BTC", startTime=1681923833000)
        else:
            response = await info.funding_history(name="BTC", startTime=1681923833000, endTime=endTime)
...
@pytest.mark.vcr()
async def test_get_l2_snapshot():
    async with Info(skip_ws=True, spot_meta=TEST_SPOT_META) as info:
        response: L2BookData = await info.l2_snapshot(name="DYDX")
    assert len(response) != 0
...
@pytest.mark.vcr()
async def test_get_candles_snapshot():
    async with Info(skip_ws=True, spot_meta=TEST_SPOT_META) as info:
        response = await info.candles_snapshot(
...
@pytest.mark.vcr()
async def test_user_funding_history_with_end_time():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_funding_history(
...
@pytest.mark.vcr()
async def test_user_funding_history_without_end_time():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_funding_history(user="0xb7b6f3cea3f66bf525f5d8f965f6dbf6d9b017b2", startTime=1681923833000)
...
async def test_historical_orders():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.historical_orders(user="0x31ca8395cf837de08b24da3f660e77761dfb974b")
...
@pytest.mark.vcr()
async def test_user_non_funding_ledger_updates_with_end_time():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_non_funding_ledger_updates(
...
@pytest.mark.vcr()
async def test_user_non_funding_ledger_updates_without_end_time():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_non_funding_ledger_updates(
...
async def test_portfolio():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.portfolio(user="0x31ca8395cf837de08b24da3f660e77761dfb974b")
    assert isinstance(response, list), "The response should be a list"
    if len(response) > 0:
...
async def test_user_twap_slice_fills():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_twap_slice_fills(user="0x31ca8395cf837de08b24da3f660e77761dfb974b")
    assert isinstance(response, list), "The response should be a list"
    if len(response) > 0:
        fill = response[0]
...
async def test_user_vault_equities():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_vault_equities(user="0x2b804617c6f63c040377e95bb276811747006f4b")
    ...
    if len(response) > 0:
        ...
        assert "equity" in vault_equity


@pytest.mark.vcr()
async def test_user_role():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_role(user="0x31ca8395cf837de08b24da3f660e77761dfb974b")
...
async def test_user_rate_limit():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.user_rate_limit(user="0x31ca8395cf837de08b24da3f660e77761dfb974b")
    assert isinstance(response, dict), "The response should be a dictionary"
    assert response is not None


@pytest.mark.vcr()
async def test_delegator_history():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.delegator_history(user="0x2ba553d9f990a3b66b03b2dc0d030dfc1c061036")
    assert isinstance(response, list), "The response should be a list"
...
@pytest.mark.vcr()
async def test_extra_agents():
    async with Info(skip_ws=True, meta=TEST_META, spot_meta=TEST_SPOT_META) as info:
        response = await info.extra_agents(user="0xd42f2bB0e06455eDB652e27b7374FC2bDa8448ee")
    assert isinstance(response, list), "The response should be a list"
